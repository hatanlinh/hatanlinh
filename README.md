# Linh Ha
- Typical corporate slave who builds some interesting security products at [OPSWAT](https://opswat.com).
- Low-level programming enthusiast at night.

### Current interests
- Linux system programming.
- High-performance computing and optimization.
- Advanced operating systems and networking topics.
- File format parsing and security.

### About me
- Used to be very lazy, which explains the empty GitHub history 😅.
- Trying to force myself into a better habit at the moment, learning and building new things every day.

### Contact
- You can check out my tech gallery at [fseek.net](https://fseek.net) and my portfolio at [hatanlinh.vn](https://hatanlinh.vn).
- You can send me an email at either `linh[at]fseek.net` or `linh[at]hatanlinh.vn`.

<!--
**hatanlinh/hatanlinh** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
